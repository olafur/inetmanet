...
enum Commands {CREATE, SETDEST, DESTROY, UNKNOWN};
...
class TraceEntryParser
{
  public:
    ...
    int command() const {return _eCmd;}
    simtime_t time() const {return SimTime::parse(_time.c_str());}
    string nodeID() const {return _nodeID;}
    
    double x() const {return atof(_x.c_str());}
    double y() const {return atof(_y.c_str());}
    double z() const {return atof(_z.c_str());}
    double speed() const {return atof(_speed.c_str());}
    simtime_t timeAtDest() const {return SimTime::parse(_timeAtDest.c_str());};
